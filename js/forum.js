...
setTimeout(()=>{
  ...
  <img src="${item.image}" class="rounded-full w-[70px]" />
    ...
    <div class="upper-title flex justify-between">
        ...
        <h3 class="text-[20px] lg:text-[20px] font-semibold mt-2 lg:mt-2">${item.title}</h3>
        <p class="text-[#12132D99] font-semibold my-3 text-[20px] lg:[text-20px]">${item.description}</p>
